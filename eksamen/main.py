import cv2
from RobotMovement import connect_to_robot, move_to_position
...
while True:
    ...
    if focus_method == "size":
        ...
        cv2.imwrite("picture_size.png", processed_frame_size)
        picture_size = cv2.imread("picture_size.png")
        cv2.imshow("Test", picture_size)
        cv2.waitKey(1)


        if size_detected:
            robot_moved_to_default = pick_and_place_based_on_size(largest_contour, robot_socket, robot_moved_to_default)
